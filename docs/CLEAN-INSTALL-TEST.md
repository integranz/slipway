...
| C3 | Ask the agent to run `echo approve-apply-probe` (first in a repository without `.cursor/hooks.json`, then in the rendered repository) | blocked both times, message starts `slipway guard:`; on 22 Sep, before the user-hooks wiring, it printed |
