...
| **Allows** | `plan`, `validate`, `fmt`, `init`, `show`, `output`; `apply <planfile>` in `infra/foundation` when a matching approval token exists (then consumes the token). **Attended session without a token** (`CLAUDE_CODE_SESSION_ATTENDED=1` in Claude Code, `SLIPWAY_SESSION_ATTENDED=1` in Cursor; permission mode not `bypassPermissions`/`dontAsk`): returns `ask` with the plan summary (`terraform show` of the plan file) as the reason, so the human approves in the permission prompt; unattended without a token: denied with the token instructions. **`options.apply_gate: token`** (or `SLIPWAY_APPLY_GATE=token`) makes the token mandatory even when attended (added 2026-09-24, 1.4.0) |
| **Human approval** | In a separate terminal: `bash <plugin-root>/scripts/approve-apply.sh infra/foundation/tfplan.dev`. Writes `.slipway/approvals/<sha256 of plan>` with a 10-minute expiry (`SLIPWAY_APPROVAL_TTL` to change). Single use: the first allowed apply renames it atomically to `<sha>.used`; other deliveries of the **same** tool call (Cursor runs the plugin, user and project hook copies in parallel) find the marker within `SLIPWAY_APPROVAL_REPLAY_SECONDS` (15) and get the same allow; a stale marker is removed and the apply denied again (added 2026-09-23). A changed plan file has a different hash and needs a new approval. |
...
- After an allowed apply, `.slipway/approvals/` holds at most a `<sha>.used` marker (removed on the next check or by the next `approve-apply.sh`); a leftover *token* means the guard did not run.
...
- **The apply asks in Cursor too (since 1.4.0).** Observed live on 2026-09-23/24 (Cursor 3.21.16, owner's launch): the adapter's `ask` for `setup-azure.sh --apply`, the environment `PUT` and the secret writes made Cursor prompt the user, and the commands ran after the approval (postToolUse four minutes later for the setup script). So the apply guard now runs attended in Cursor like the other guards and returns `ask` with the plan summary; a background agent (`is_background_agent`) still gets the token path, and `options.apply_gate: token` makes the token mandatory for repositories where Cursor's auto-run may be on. Before 1.4.0 the adapter forced the token path because Cursor documents `ask` as unenforced for `preToolUse`.
...
**Update 2026-09-23 (owner's Cursor after a restart):** third-party plugin import became enabled, and Cursor then loaded the plugin's own `cursor/hooks.json` too ("claude-plugin config", three registrations: local folder, marketplace cache, Claude Code cache) next to the user and project sources: five adapter runs per tool call, all `allow`/`deny` consistently, plus the imported `endor-fix` Claude Code plugin's guards, which denied every command because their CLI is missing in Cursor's copy (`Hook blocked … endor-fix-hook: CLI not found`). Consequences: the approval token is now consumed atomically with a 15-second replay window for the same call (guard 1), and the README tells users to disable a foreign plugin whose hooks deny everything. The user and project sources stay: they are the only path when an admin disables third-party imports.
