...
Still to verify live on 25 Sep: whether `beforeShellExecution`, `beforeReadFile` and `sessionStart` are requested at all in 3.21.x (the build contains them), the real input keys of the edit and read tools, and whether `ask` prompts. The probe covers the wiring: in a fresh Cursor session, `echo approve-apply-probe` must be blocked with a message starting `slipway guard:`.
