...
Branch tests: `bash plugins/slipway/hooks/test-hooks.sh` (117 cases, run in CI by `plugin-ci.yml`). Last local run: 2026-09-22, `passed=117 failed=0`. The Cursor adapters have their own 43 cases (`plugins/slipway/cursor/hooks/test-cursor-hooks.sh`, section 6).
...
Tests: `bash plugins/slipway/cursor/hooks/test-cursor-hooks.sh`, 43 cases with Cursor-shaped inputs (apply with and without token, destroy, app layer, self-approval, probe, docker push, `git add .env`, seed file read/print/source, bare `env`, admin writes attended → `ask` and unattended → `deny`, MCP `run_workflow` tags, edit tools with alternative keys, malformed input, crashing and missing guards, session-start context). Last local run 2026-09-22: `passed=43 failed=0`. The manifests and generated assets are checked by `node --test scripts/cursor-plugin.test.cjs` (9 tests); both run in CI (`npm run test:cursor`).

Not yet verified in a live Cursor session (planned for the 25 Sep clean-install test, which runs in Cursor): that plugin hook commands resolve relative to the plugin root as in the docs example (`./cursor/hooks/shell.sh`); that `sessionStart` `env` reaches the later hooks; the real input keys of Cursor's edit tools; whether `ask` prompts for `beforeShellExecution`. The install-time probe covers the first two mechanically: in a fresh Cursor session, `echo approve-apply-probe` must be blocked with a message starting `slipway guard:`.
