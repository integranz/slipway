...
Guardrails are enforced by the plugin's hooks: a local 'terraform apply' needs a saved plan in infra/foundation and a human approval token created by the human in their own terminal with: bash \"$PLUGIN_ROOT/scripts/approve-apply.sh\" <planfile>. Never terraform destroy or -auto-approve; the app layer (infra/apps/*) is applied only by its CD workflow. Never commit .env*, *.tfvars, state or plan files; only immutable image tags; .slipway/.env is sourced, never printed."
python3 -c '
import json,sys
