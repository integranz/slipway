...
## Any stack
`stack: custom` accepts any language: you bring the Dockerfile, slipway verifies the contract (VERSION/COMMIT build args, health endpoint reporting the version, non-root runtime) and does the rest unchanged. Curated hardened templates exist for .NET 8 APIs and React/Vite frontends. Apps may live at the repository root (`path: .`). Per-app `test_services` start service containers (for example PostgreSQL) next to the tests.
