...
- Pre-fill from arguments, then from detections (e.g. a `version.json` → `nbgv`, a `.releaserc` → `semantic-release`, `*.csproj` → `dotnet8-api`, `package.json` with `vite` → `react-vite`), then from the defaults table above. Any other language or a Node API gets `stack: custom` (see `delivery-knowledge/references/stack-custom.md`): the Dockerfile is written by the agent, not by the user; if none exists, the scaffold stops with a clear message, so draft one first (execute sub-agent), show it, ask before writing it, and re-run the scaffold. `node-ts-api`, the configuration scanner and database provisioning are planned for 1.1: say so when they come up, never pretend they exist.
...
- `registry_scope` (with `registry: acr`) is **discovered, not guessed**: run `node "${CLAUDE_PLUGIN_ROOT}/scripts/registries.cjs" --location <azure.location> --json` (read-only: lists the subscription's registries and whether the signed-in identity can assign the roles the foundation needs on each: AcrPush and Reader for the CI/CD principal, AcrPull for the app identity). Then: `recommendation: per-repository` with an **empty** list → set `per-repository` and tell the user that no registry exists and the foundation will create `<azure.acr_name>`, no question. `recommendation: ask` → `AskUserQuestion` with one option per registry in `eligible` (label `<name> · <resource group> · <location> · <sku>`, mark `same_location`), plus **"Create a new registry for this repository"**; registries with `can_assign_roles: false` go into the question text as not selectable, with the reason ("you cannot assign roles on it; ask an owner for AcrPush, Reader and AcrPull for `sp-<project>-github` and the app identity, or Owner on it"). A chosen registry → `registry_scope: existing`, `azure.acr_name` = its name, `azure.acr_resource_group` = its group (omit when equal to `azure.resource_group`). `recommendation: per-repository` with registries listed but none eligible → set `per-repository`, show the list and the reason. When the script exits 2 (no `az login`) say so and ask the two-way question directly. Never switch a delivered repository from per-repository to existing without the human handling the state: the plan would destroy its registry and every image in it.
