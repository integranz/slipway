...
- `registry_scope` (with `registry: acr`): **per-repository** (default; the foundation layer creates the registry inside this repository's resource group) or **existing** (a registry shared by several repositories). With `existing`, ask for the registry name and its resource group (`azure.acr_name`, `azure.acr_resource_group` when it differs from `azure.resource_group`), verify it with `az acr show -n <name> -g <group> --query id -o tsv`, and tell the user that the person applying the foundation needs role-assignment rights on that registry (the layer assigns AcrPush, Reader and AcrPull there). Images stay namespaced `<project>/<app>`. Never switch a delivered repository from per-repository to existing without the human handling the state: the plan would destroy its registry and every image in it.
