...
- Discovery: `scripts/registries.cjs [--location <region>] [--json]` lists the subscription's registries with `can_assign_roles` (true when the signed-in identity holds Owner, User Access Administrator or Role Based Access Control Administrator on the registry, its group or the subscription, read with `az role assignment list --assignee … --all --include-inherited --include-groups`), `same_location`, and a `recommendation`: `per-repository` when there is no registry (create one, no question) or none you can grant on; `ask` otherwise (offer the eligible ones and "create a new one").
