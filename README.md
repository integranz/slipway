...
| MCP servers | `.mcp.json`, authorised with `/mcp` | `cursor/mcp.json`: Atlassian and Azure connect as in Claude Code; **GitHub needs a token**: Cursor cannot complete GitHub's OAuth for this server (no dynamic client registration), so set the plugin variable `GITHUB_MCP_TOKEN` (Settings → Plugins → slipway → Configure) to a fine-grained PAT with Actions read/write and Metadata read on your repositories. Optional: the skills use the `gh` CLI when the server is absent |
