...
| MCP servers | `.mcp.json`, authorised with `/mcp` | `cursor/mcp.json`: Atlassian, Azure and GitHub connect from the plugin (verified in Cursor 3.21.18: Atlassian 21 tools, GitHub 4 tools; OAuth tokens are refreshed silently, so a server that connected once does not prompt again). Cursor 3.21.16 could not authenticate the GitHub server (no dynamic client registration); for such builds the optional plugin variable `GITHUB_MCP_TOKEN` (a fine-grained PAT, set under Plugins → Configure where that UI exists) is sent as a bearer token. The skills use the `gh` CLI whenever the server is absent |
